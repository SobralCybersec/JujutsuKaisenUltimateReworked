...
public class SpawnFakeCloneProcedure {
    public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
        if (entity == null)
            return;
        if (((entity.getCapability(JujutsucraftaddonModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new JujutsucraftaddonModVariables.PlayerVariables())).Clans.equals("Majima"))) {
            if (world instanceof ServerLevel _serverLevel) {
                LevelAccessor _worldorig = world;
                world = _serverLevel.getServer().getLevel(entity.level().dimension());
                if (world != null) {
                    Entity entitytospawn = JujutsucraftaddonModEntities.PURPLE_CLONES.get().create(_serverLevel, null, null, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED, false, false);
                    if (entitytospawn != null) {
                        ((FakePurpleClonesEntity) entitytospawn).getEntityData().set(FakePurpleClonesEntity.ATTACK_MODE, true);
                        ...
                        ((FakePurpleClonesEntity) entitytospawn).setOwnerUUID(entity.getUUID());
                        entitytospawn.getPersistentData().putString("Owner", entity.getStringUUID());
                        (entitytospawn).setCustomName(Component.literal((entity.getDisplayName().getString() + " §0Majima Clone")));
                        ...
                        if ((entitytospawn) instanceof LivingEntity _entity && !_entity.level().isClientSide())
                            _entity.addEffect(new MobEffectInstance(MobEffects.HEAL, 60, 245, false, false));
                        if ((entitytospawn) instanceof LivingEntity _entity && !_entity.level().isClientSide())
                            _entity.addEffect(new MobEffectInstance(MobEffects.REGENERATION, 60, 245, false, false));
                        if ((entitytospawn) instanceof LivingEntity _entity && !_entity.level().isClientSide())
                            _entity.addEffect(new MobEffectInstance(JujutsucraftModMobEffects.PHYSICAL_GIFTED_EFFECT.get(), -1, 1, false, false));
                        {
                            CompoundTag dataIndex = new CompoundTag();
                            (entitytospawn).saveWithoutId(dataIndex);
            ...
            if (world instanceof ServerLevel _serverLevel) {
                ...
                if (world != null) {
                    Entity entitytospawn = JujutsucraftaddonModEntities.FAKE_CLONES.get().create(_serverLevel, null, null, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED, false, false);
                    if (entitytospawn != null) {
                        if (((entity.getCapability(JujutsucraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new JujutsucraftModVariables.PlayerVariables())).PlayerCurseTechnique2 == 100)) {
                            if (entitytospawn instanceof FakeClonesEntity _datEntSetL)
                                _datEntSetL.getEntityData().set(FakeClonesEntity.ATTACK_MODE, true);
