...
public class WukongSkillMenu extends Screen {
    ...
    protected void init() {
        ...
        this.pages.clear();
        List<WukongSkillMenu.AltarOption> options = this.getOptions();
    ...
    private List<WukongSkillMenu.AltarOption> getOptions() {
        List<WukongSkillMenu.AltarOption> options = new ArrayList<>();
...
//        options.add(new WukongScreen.AltarOption("Song of Sun"));
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < this.getCurrent().size(); i++) {
            ...
            WukongSkillMenu.AltarOption item = this.getCurrent().get(i);
    ...
    private List<? extends WukongSkillMenu.AltarOption> getCurrent() {
    ...
    public void onClose() {
        ...
        List<? extends WukongSkillMenu.AltarOption> items = this.pages.get(page);

        if (this.hovered >= items.size()) return;

        WukongSkillMenu.AltarOption item = items.get(this.hovered);
        item.onSelect();
    }
