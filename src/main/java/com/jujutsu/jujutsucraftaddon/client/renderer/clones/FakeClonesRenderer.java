package com.jujutsu.jujutsucraftaddon.client.renderer.clones;

import com.jujutsu.jujutsucraftaddon.entity.clone.FakeClonesEntity;
import com.jujutsu.jujutsucraftaddon.entity.model.FakeCloneModel;
...
public class FakeClonesRenderer extends DynamicGeoEntityRenderer<FakeClonesEntity> {
    ...
    private GeoModel<FakeClonesEntity> currentModel;

    public FakeClonesRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new FakeCloneModel());
    ...
    @Override
    public RenderType getRenderType(FakeClonesEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(getTextureLocation(animatable));
    }

    @Override
    public void preRender(PoseStack poseStack, FakeClonesEntity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green,
...
//    @Override
//    public void render(FakeClonesEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
//        if (entity.getOwnerUUID() != null) {
...
//    }
//
//    private GeoModel<FakeClonesEntity> getModel(UUID uuid) {
...
class FakeClonesRenderer2 extends BlockAndItemGeoLayer<FakeClonesEntity> {

    private final FakeClonesRenderer FakeClonesRenderer;

    public FakeClonesRenderer2(FakeClonesRenderer FakeClonesRenderer, GeoRenderer<FakeClonesEntity> renderer) {
        super(renderer);
        this.FakeClonesRenderer = FakeClonesRenderer;
    }

    @Nullable
    @Override
    protected ItemStack getStackForBone(GeoBone bone, FakeClonesEntity animatable) {
        return switch (bone.getName()) {
            case "HandLeft" -> FakeClonesRenderer.getAnimatable().getMainHandItem();
            case "HandRight" -> FakeClonesRenderer.getAnimatable().getOffhandItem();
            default -> null;
        };
    }

    @Override
    protected ItemDisplayContext getTransformTypeForStack(GeoBone bone, ItemStack stack, FakeClonesEntity animatable) {
        return switch (bone.getName()) {
    ...
    @Override
    protected void renderStackForBone(PoseStack poseStack, GeoBone bone, ItemStack stack, FakeClonesEntity animatable, MultiBufferSource bufferSource, float partialTick, int packedLight, int packedOverlay) {
        if (stack.equals(FakeClonesRenderer.getAnimatable().getMainHandItem())) {
            poseStack.mulPose(Axis.XP.rotationDegrees(-90.0F));
            if (stack.getItem() instanceof ShieldItem) {
                poseStack.translate(0.0, 0.125, -0.25);
            }
        } else if (stack.equals(FakeClonesRenderer.getAnimatable().getOffhandItem())) {
            poseStack.mulPose(Axis.XP.rotationDegrees(-90.0F));
            if (stack.getItem() instanceof ShieldItem) {
                poseStack.translate(0.0, 0.125, 0.25);
...
class FakeClonesRenderer3 extends ItemArmorGeoLayer<FakeClonesEntity> {
    private final FakeClonesRenderer FakeClonesRenderer2;

    public FakeClonesRenderer3(FakeClonesRenderer FakeClonesRenderer2, GeoRenderer<FakeClonesEntity> geoRenderer) {
        super(geoRenderer);
        this.FakeClonesRenderer2 = FakeClonesRenderer2;
    }

    @Nullable
    protected ItemStack getArmorItemForBone(GeoBone bone, FakeClonesEntity animatable) {
        ItemStack var10000;
        switch (bone.getName()) {
            case "armorLeftFoot":
            case "armorRightFoot":
    ...
    @Nonnull
    protected EquipmentSlot getEquipmentSlotForBone(GeoBone bone, ItemStack stack, FakeClonesEntity animatable) {
    ...
    @Nonnull
    protected ModelPart getModelPartForBone(GeoBone bone, EquipmentSlot slot, ItemStack stack, FakeClonesEntity animatable, HumanoidModel<?> baseModel) {
        ModelPart var10000;
