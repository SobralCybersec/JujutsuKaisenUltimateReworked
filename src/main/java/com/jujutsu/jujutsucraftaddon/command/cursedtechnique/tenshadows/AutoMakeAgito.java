...
public class AutoMakeAgito extends BaseCommand<Double> {

    @Override
    protected String getName () {
        return "jjkuTenShadowsAgito";
    }
    ...
    protected void execute (CommandContext<CommandSourceStack> ctx, Double value){
        AutoMakeProcedure.execute(ctx.getSource().getUnsidedLevel(), ctx.getSource().getEntity());
    }

    @SubscribeEvent
    public static void registerCommand (RegisterCommandsEvent event){
        new AutoMakeAgito().register(event);
    }
