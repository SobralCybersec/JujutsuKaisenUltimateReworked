...
import com.jujutsu.jujutsucraftaddon.procedures.KoganeItemProcedure;
...
public class PVPKogane extends BaseCommand<Double> {

    @Override
    protected String getName () {
        return "jjkuPVPKogane";
    }
    ...
    protected void execute (CommandContext<CommandSourceStack> ctx, Double value){
        KoganeItemProcedure.execute(ctx.getSource().getUnsidedLevel(), ctx.getSource().getEntity());
    }

    @SubscribeEvent
    public static void registerCommand (RegisterCommandsEvent event){
        new PVPKogane().register(event);
    }
}
