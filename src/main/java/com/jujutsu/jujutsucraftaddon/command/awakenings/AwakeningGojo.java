...
import com.jujutsu.jujutsucraftaddon.procedures.SokaProcedure;
...
public class AwakeningGojo extends BaseCommand<Double> {

    @Override
    protected String getName () {
        return "jjkuAwakeningGojo";
    }
    ...
    protected void execute (CommandContext<CommandSourceStack> ctx, Double value){
        SokaProcedure.execute(ctx.getSource().getEntity());
    }

    @SubscribeEvent
    public static void registerCommand (RegisterCommandsEvent event){
        new AwakeningGojo().register(event);
    }
}
