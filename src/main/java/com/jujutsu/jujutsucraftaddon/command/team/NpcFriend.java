...
public class NpcFriend extends BaseCommand<Double> {

    @Override
    protected String getName() {
        return "jjkuTeamNpcInvite";
    }
    ...
    protected void execute(CommandContext<CommandSourceStack> ctx, Double value) {
        NpcTeamInvite.execute(ctx, ctx.getSource().getEntity());
    }

    @SubscribeEvent
    public static void registerCommand(RegisterCommandsEvent event) {
        new NpcFriend().register(event);
    }
}
